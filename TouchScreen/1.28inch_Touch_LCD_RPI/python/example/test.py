...
drink_data =    [{'name': 'Margarita', 'primary_color': 'LIME','secondary_color': 'WHITE', 'x': 25, 'y':90},
                ...
                {'name':'Manhattan', 'primary_color':'coral','secondary_color':'BLACK', 'x': 14, 'y':90},
                {'name':'Mimosa', 'primary_color':'LIME','secondary_color':'OrangeRed', 'x': 45, 'y':90},
                {'name':'Screwdriver', 'primary_color':'coral','secondary_color':'orange', 'x': 7, 'y':90},
                ]
...
def sizeOptions():
    ...
    while dispensing == False:
        ...
        draw.rectangle((0,0,240,240),fill = "BLACK", outline=None, width=1)
        if drinkSize == 1:
            drink_size_text = 'Small'
        elif drinkSize == 2:
            drink_size_text = 'medium'
        elif drinkSize == 3:
            drink_size_text = 'large'
        draw.text((65, 90), drink_size_text, fill = "WHITE",font=Font)
        draw.text((30,180), 'hold to select', fill = "WHITE",font=smallFont)
        disp.ShowImage(image2)
        time.sleep(0.01)
    ...
    while True:
        ...
        while True:
            while touch.Gestures == 0x03 or touch.Gestures == 0x04:
                ...
                draw.text((30, 200), 'tap to select', fill = drink_data[drink_list_index]['primary_color'],font=smallFont)
                disp.ShowImage(image1)
                touch.Gestures = 0x01
                time.sleep(0.01)
